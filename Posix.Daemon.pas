...
type
  ...
  TPosixSignal = (Termination, Reload, AccessViolation);
  ...
  public
    ...
    /// </summary>
    class procedure Setup(ASignalProc: TSignalProc; const APidFileName : String);
...
begin
  case ASigNum of
    ...
    SIGSEGV:
    begin
      TPosixDaemon.LogInfo('Signal SIGSEGV received');
      TPosixDaemon.FSignalProc(TPosixSignal.AccessViolation);
    end;
  end;
end;
...
class procedure TPosixDaemon.Setup(ASignalProc: TSignalProc; const APidFileName : String);
begin
  ...
  if APidFileName <> '' then
  begin
    TFile.Delete('/var/run/'+APidFileName+'.pid');
    TFile.WriteAllText('/var/run/'+APidFileName, TPosixDaemon.ProcessID.ToString);
  end;
